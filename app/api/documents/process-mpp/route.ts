...
export async function POST(req: NextRequest) {
  try {
    ...
    const docs = await query<{ id: string; project_id: string; storage_path: string; file_name: string }>(
      documentId
        ? 'SELECT id, project_id, storage_path, file_name FROM project_documents WHERE id = $1'
        : `SELECT id, project_id, storage_path, file_name
           FROM project_documents
           WHERE project_id = $1
           ORDER BY is_current_version DESC, uploaded_at DESC
           LIMIT 1`,
      [documentId || projectId],
    );
    const doc = docs[0];
    if (!doc) return NextResponse.json({ error: 'Document not found' }, { status: 404 });
    if (String(doc.project_id) !== String(projectId)) {
      return NextResponse.json({ error: 'Document does not belong to project' }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: true, processedDocumentId: doc.id, ...counts });
  } catch (err: unknown) {
