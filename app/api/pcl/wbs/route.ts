...
export async function GET(req: NextRequest) {
  try {
    ...
    const tableForType = (type: string) => {
    ...
    phases.forEach((p) => { const key = str((p as R).unit_id); if (!phaseByUnit.has(key)) phaseByUnit.set(key, []); phaseByUnit.get(key)!.push(p as R); });
    const taskByPhase = new Map<string, R[]>();
    tasks.forEach((t) => { const key = str((t as R).phase_id); if (!taskByPhase.has(key)) taskByPhase.set(key, []); taskByPhase.get(key)!.push(t as R); });
    const subTaskByTask = new Map<string, R[]>();
    subTasks.forEach((s) => { const key = str((s as R).task_id); if (!subTaskByTask.has(key)) subTaskByTask.set(key, []); subTaskByTask.get(key)!.push(s as R); });
    ...
    const buildActualHoursTooltip = (row: R, type: string) => {
    ...
    function toRow(row: R, type: string, level: number, parentId: string | null, hasChildren: boolean) {
      ...
      return {
        id: str(row.id), parent_id: parentId, type, level, has_children: hasChildren,
        project_id: str(row.project_id), unit_id: str(row.unit_id),
        phase_id: str(row.phase_id), task_id: str(row.task_id || row.id),
        wbs_code: str(row.wbs_code), name: str(row.name),
        ...
        baseline_start: row.baseline_start || null, baseline_end: row.baseline_end || null,
        days_required: num(row.days), baseline_hours: baselineHours,
        actual_hours: actualHours, remaining_hours: remainingHours, work,
        baseline_cost: baselineCost, actual_cost: actualCost, remaining_cost: remainingCost,
        schedule_cost: scheduleCost, efficiency, percent_complete: percentComplete,
        ...
        relationship: str(row.relationship), lag_days: num(row.lag_days),
        total_float: num(row.total_float || row.tf),
        is_critical: Boolean(row.is_critical), is_milestone: Boolean(row.is_milestone), cpi,
    ...
    (units as R[]).forEach((u) => { const key = str(u.project_id); if (!unitByProject.has(key)) unitByProject.set(key, []); unitByProject.get(key)!.push(u); });
    ...
    const ensureSet = (map: Map<string, Set<string>>, key: string) => { if (!map.has(key)) map.set(key, new Set<string>()); return map.get(key)!; };

    (projects as R[]).forEach((p) => {
      const pid = str(p.id);
      if (usedProjectIds.size > 0 && !usedProjectIds.has(pid)) return;
      const portId = str(p.portfolio_id) || '__unassigned_portfolio__';
      const custId = str(p.customer_id) || `__unassigned_customer__:${portId}`;
      const siteId = str(p.site_id) || `__unassigned_site__:${custId}`;
      treePortfolios.set(portId, portfolioMap.get(portId) || { id: portId, name: 'Unassigned Portfolio' });
      treeCustomers.set(custId, customerMap.get(custId) || { id: custId, name: 'Unassigned Customer', portfolio_id: portId });
      treeSites.set(siteId, siteMap.get(siteId) || { id: siteId, name: 'Unassigned Site', customer_id: custId, portfolio_id: portId });
      treeProjects.set(pid, p);
      ensureSet(customerIdsByPortfolio, portId).add(custId);
      ensureSet(siteIdsByCustomer, custId).add(siteId);
      ensureSet(projectIdsBySite, siteId).add(pid);
    });
    ...
    for (const portfolioId of sortedPortfolioIds) {
      const customerIds = [...(customerIdsByPortfolio.get(portfolioId) || [])];
      ...
      for (const customerId of customerIds) {
        const siteIds = [...(siteIdsByCustomer.get(customerId) || [])];
        ...
        for (const siteId of siteIds) {
          const projectIds = [...(projectIdsBySite.get(siteId) || [])];
