      ...
      <div className="chart-card" style={{ marginBottom: '1rem', minHeight: '600px' }}>
        ...
        <div className="chart-card-body no-padding" style={{ height: 'calc(100% - 60px)', overflow: 'auto' }}>
          <table className="data-table" id="table-labor-worker" style={{ fontSize: '0.75rem' }}>
            <thead>
              <tr>
                <th style={{ position: 'sticky', left: 0, top: 0, background: 'var(--bg-secondary)', zIndex: 10 }}>Worker</th>
                <th style={{ position: 'sticky', top: 0, background: 'var(--bg-secondary)', zIndex: 5 }}>Role</th>
                <th style={{ position: 'sticky', top: 0, background: 'var(--bg-secondary)', zIndex: 5 }}>Project</th>
                ...
                <th className="number" style={{ fontWeight: 700, position: 'sticky', top: 0, background: 'var(--bg-secondary)', zIndex: 5 }}>Total</th>
              </tr>
            </thead>
            <tbody>
              {filteredLaborBreakdown.length > 0 ? (
                filteredLaborBreakdown.map((worker, idx) => (
                  <tr key={idx}>
      ...
      <div className="chart-card" style={{ marginBottom: '1rem', minHeight: '400px' }}>
        ...
        <div className="chart-card-body no-padding" style={{ height: 'calc(100% - 60px)', overflow: 'auto' }}>
          <table className="data-table" id="table-labor-role" style={{ fontSize: '0.75rem' }}>
            <thead>
              <tr>
                <th style={{ position: 'sticky', left: 0, top: 0, background: 'var(--bg-secondary)', zIndex: 10 }}>Role</th>
                <th style={{ position: 'sticky', top: 0, background: 'var(--bg-secondary)', zIndex: 5 }}>Employees</th>
                {tableWeeks.map((week, idx) => (
                  <th key={idx} className="number" style={{ minWidth: '65px', position: 'sticky', top: 0, background: 'var(--bg-secondary)', zIndex: 5 }}>
                    {formatWeekLabel(week)}
                  </th>
            ...
            <tbody>
              {roles.length > 0 ? (
                roles.map((role, idx) => {
                  ...
                  // Aggregate hours by week for this role
                  const weeklyTotals = tableWeeks.map((_, weekIdx) => {
                    return roleWorkers.reduce((sum, w) => sum + ((w.data || [])[weekIdx] || 0), 0);
                  });
                  const roleTotal = weeklyTotals.reduce((sum, h) => sum + h, 0);
                  
                  return (
                    <tr key={idx}>
                      <td style={{ position: 'sticky', left: 0, background: 'var(--bg-primary)', zIndex: 5, fontWeight: 500 }}>{role}</td>
                      <td>{employeeCount}</td>
                      {weeklyTotals.map((hours, weekIdx) => (
