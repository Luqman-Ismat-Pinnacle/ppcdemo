...
export default function MosPage() {
  ...
  const noteScope = useMemo(() => {
    const path = hierarchyFilter?.path || [];
    return {
      portfolioId: hierarchyFilter?.portfolioId || (path[0] ? String(byName(portfolios, path[0])?.id || '') : '') || null,
      customerId: hierarchyFilter?.customerId || (path[1] ? String(byName(customers, path[1])?.id || '') : '') || null,
      siteId: hierarchyFilter?.siteId || (path[2] ? String(byName(sites, path[2])?.id || '') : '') || null,
      projectId: hierarchyFilter?.projectId || (path[3] ? String(byName(projects, path[3])?.id || '') : '') || null,
    };
        ...
        <>
          ...
          <section style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 12, padding: '0.8rem', display: 'grid', gap: '0.7rem' }}>
            <h3 style={{ margin: 0, color: C.text, fontSize: '0.95rem' }}>
              ...
              <MetricProvenanceChip provenance={periodVarianceProvenance} value={`${periodHours.efficiency}%`} onExplain={onExplain} />
            </h3>
