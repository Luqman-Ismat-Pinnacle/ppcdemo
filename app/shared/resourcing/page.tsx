    ...
    return (
      <div className="page-panel" style={{ padding: '2rem' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4rem 2rem', background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--border-color)', textAlign: 'center' }}>
          ...
          <a href="/shared/data-management" style={{ padding: '0.75rem 1.5rem', background: 'var(--pinnacle-teal)', color: '#000', borderRadius: '8px', textDecoration: 'none', fontWeight: 600, fontSize: '0.9rem' }}>Go to Data Management</a>
        </div>
      </div>
    );
  }
            ...
            {[
              ...
              <div key={m.label} style={{ padding: '0.75rem', background: m.bg || 'var(--bg-card)', borderRadius: '10px', border: `1px solid ${m.border || 'var(--border-color)'}` }}>
                <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 6 }}>
                  {m.label}
                  {m.provenance ? <MetricProvenanceChip provenance={m.provenance} value={m.value} onExplain={onExplain} /> : null}
                </div>
