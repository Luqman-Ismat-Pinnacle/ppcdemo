              ...
              {virtualRows.map((row) => {
                ...
                return (
                  <tr key={row.id} style={{ height: `${rowHeight}px`, background: isCritical ? 'rgba(220, 38, 38, 0.05)' : 'var(--bg-primary)' }}>
                    ...
                    <td className="number" style={{ fontSize: '0.6rem' }}>{row.daysRequired != null && isFinite(Number(row.daysRequired)) ? Number(row.daysRequired).toFixed(0) : '-'}</td>
                    <td className="number" style={{ fontSize: '0.6rem' }}>{row.baselineHours && isFinite(Number(row.baselineHours)) ? Number(row.baselineHours).toFixed(0) : '-'}</td>
                    <td className="number" style={{ fontSize: '0.6rem', color: 'var(--pinnacle-teal)' }}>{row.actualHours && isFinite(Number(row.actualHours)) ? Number(row.actualHours).toFixed(0) : '-'}</td>
                    <td className="number" style={{ fontSize: '0.6rem' }}>{(row as any).remainingHours != null && isFinite(Number((row as any).remainingHours)) ? Number((row as any).remainingHours).toFixed(0) : '-'}</td>
                    <td className="number" style={{ fontSize: '0.6rem' }}>{formatCurrency(Number(row.baselineCost))}</td>
                    <td className="number" style={{ fontSize: '0.6rem', color: 'var(--pinnacle-teal)' }}>{formatCurrency(Number(row.actualCost))}</td>
                    <td className="number" style={{ fontSize: '0.6rem' }}>{formatCurrency(Number((row as any).remainingCost))}</td>
