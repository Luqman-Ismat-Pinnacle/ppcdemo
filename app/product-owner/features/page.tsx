...
const STAGES = ['planned', 'triaged', 'in_progress', 'resolved', 'released', 'closed'];

export default function ProductOwnerFeaturesPage() {
  ...
  const loadItems = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch('/api/feedback?type=feature&limit=300', { cache: 'no-store' });
  ...
  const visible = useMemo(() =>
    items.filter(i => stageFilter === 'all' || i.status === stageFilter),
  [items, stageFilter]);
    ...
    <div className="page-panel" style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Track feature requests from planning through release.</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.65rem' }}>
        ...
        <StatCard label="Planned" value={summary.planned} accent="#60A5FA" />
        <StatCard label="In Progress" value={summary.inProgress} accent="#8B5CF6" />
        <StatCard label="Released" value={summary.released} accent="#22C55E" />
      </div>

      <div style={{ display: 'flex', gap: '0.35rem', flexWrap: 'wrap', alignItems: 'center' }}>
        <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginRight: '0.3rem' }}>Stage:</span>
        {['all', ...STAGES].map(s => (
          <button key={s} type="button" onClick={() => setStageFilter(s)} style={pillStyle(stageFilter === s)}>
      ...
      {loading ? (
        <div style={{ padding: '1.5rem', color: 'var(--text-muted)', fontSize: '0.8rem' }}>Loading features...</div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: '62vh', overflow: 'auto' }}>
          {visible.length === 0 && <div style={{ padding: '1rem', color: 'var(--text-muted)' }}>No features match this filter.</div>}
          {visible.map(item => (
            <div key={item.id} style={{ border: '1px solid var(--border-color)', borderRadius: 10, background: 'var(--bg-secondary)', padding: '0.7rem 0.8rem' }}>
              ...
              <div style={{ fontSize: '0.74rem', color: 'var(--text-secondary)', marginBottom: '0.45rem', whiteSpace: 'pre-wrap', maxHeight: 100, overflow: 'auto' }}>
                {item.description}
              </div>
              ...
              {item.notes && <div style={{ fontSize: '0.62rem', color: 'var(--text-muted)', marginTop: '0.35rem', whiteSpace: 'pre-wrap' }}>{item.notes}</div>}
            </div>
          ))}
        </div>
      )}
