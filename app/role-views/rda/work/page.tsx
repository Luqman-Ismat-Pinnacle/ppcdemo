...
export default function RdaWorkPage() {
  ...
  const rows = useMemo(() => {
    ...
    }).filter((row) => row.progress < 100).sort((a, b) => Number(b.overdue) - Number(a.overdue)).slice(0, 120);
  ...
  return (
    <RoleWorkstationShell role="rda" title="Work Lane" subtitle="Task-level execution lane for daily progress operations.">
      ...
      <div style={{ border: '1px solid var(--border-color)', borderRadius: 12, background: 'var(--bg-card)', overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 120px 110px 130px 130px', padding: '0.5rem 0.7rem', borderBottom: '1px solid var(--border-color)', fontSize: '0.68rem', color: 'var(--text-muted)' }}>
        ...
        {rows.length === 0 ? (
          ...
          <div key={row.id} style={{ display: 'grid', gridTemplateColumns: '1.4fr 120px 110px 130px 130px', padding: '0.55rem 0.7rem', borderBottom: '1px solid var(--border-color)', fontSize: '0.74rem' }}>
            <span>{row.name}</span>
            ...
            <span style={{ color: 'var(--text-secondary)' }}>{row.overdue ? 'Update Now' : 'Update'}</span>
          </div>
        ))}
      </div>
