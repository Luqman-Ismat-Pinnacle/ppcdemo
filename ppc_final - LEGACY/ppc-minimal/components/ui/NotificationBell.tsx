...
export default function NotificationBell({ role = 'PCA' }: { role?: string }) {
  ...
  const markAllRead = async () => {
    ...
    try {
      await fetch('/api/notifications', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ markAllRead: true, role }) });
    } catch { /* silent */ }
  };

  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen(!open)} aria-label="Notifications" className="nav-icon-btn">
        <svg viewBox="0 0 24 24" width="17" height="17" stroke="currentColor" strokeWidth="2" fill="none">
          <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" />
          <path d="M13.73 21a2 2 0 0 1-3.46 0" />
        </svg>
        {unreadCount > 0 && (
          <span style={{ position: 'absolute', top: 0, right: 0, minWidth: 15, height: 15, borderRadius: 10, background: '#ef4444', color: '#fff', fontSize: '0.55rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px', lineHeight: 1 }}>
      ...
      {open && (
        <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: 8, width: 360, maxHeight: 440, borderRadius: 'var(--radius-md)', background: 'rgba(12,14,18,0.92)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', border: '1px solid var(--glass-border)', boxShadow: '0 16px 48px rgba(0,0,0,0.4)', zIndex: 1100, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, overflow: 'auto' }}>
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => {
                ...
                return (
                  <div key={n.id} onClick={() => !n.isRead && markRead(n.id)} style={{ padding: '9px 14px', borderBottom: '1px solid var(--glass-border)', cursor: n.isRead ? 'default' : 'pointer', background: n.isRead ? 'transparent' : 'rgba(16,185,129,0.03)', transition: 'background 0.15s', display: 'flex', gap: 9, alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: '0.58rem', color: typeColor, fontWeight: 600, marginTop: 2, textTransform: 'capitalize' }}>{n.type.replace(/_/g, ' ')}</div>
                    </div>
                  </div>
                );
              })
