...
type ActionProject = { id: string; name: string; total_entries: string; unmapped_entries: string; mapped_pct: string };
...
export default function CommandCenterPage() {
  ...
  const [actionProjects, setActionProjects] = useState<ActionProject[]>([]);
  ...
  useEffect(() => {
    fetch('/api/pca/summary', { cache: 'no-store' })
      ...
      .then(data => {
        ...
        setActionProjects(data.actionProjects || []);
  ...
  return (
    <div>
      ...
      <p className="page-subtitle">Operational feed for immediate actions, fresh updates, and triage.</p>
      ...
      {loading ? (
        <div className="kpi-grid" style={{ marginBottom: '0.85rem' }}>
          {Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} height={76} />)}
        </div>
      ) : kpis && (
        <div className="kpi-grid" style={{ marginBottom: '0.85rem' }}>
          <KpiCard label="Open Queue Items" value={queue.length} detail="Needs PCA attention" />
          <KpiCard label="Unmapped Entries" value={kpis.unmappedHours.toLocaleString()} detail="Needs mapping" />
          <KpiCard label="Recent Activity Rows" value={activity.length} detail="Projects updated this week" />
          <KpiCard label="Hours Logged (6mo)" value={fmt(kpis.totalHoursLogged)} />
          <KpiCard label="No Schedule" value={kpis.projectsNoSchedule} detail="Active projects missing plan" />
        </div>
      )}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.65rem', marginBottom: '0.85rem' }}>
        ...
        <div className="glass-raised" style={{ padding: '0.75rem' }}>
          <div style={{ fontSize: '0.76rem', fontWeight: 700, marginBottom: '0.5rem' }}>Projects Requiring Mapping</div>
          {loading && <Skeleton height={80} />}
          {!loading && actionProjects.length === 0 && <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>No mapping backlog.</div>}
          {actionProjects.map((row) => (
            <div key={row.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.35rem 0', borderBottom: '1px solid var(--glass-border)', fontSize: '0.74rem' }}>
              <span style={{ fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '58%' }}>{row.name || row.id}</span>
              <span style={{ color: 'var(--text-secondary)', whiteSpace: 'nowrap' }}>
                {Number(row.unmapped_entries)} open · {Number(row.mapped_pct || 0).toFixed(0)}%
              </span>
            </div>
          ))}
        </div>
      ...
      <div className="glass-raised" style={{ padding: '0.75rem', marginBottom: '0.85rem' }}>
        <div style={{ fontSize: '0.76rem', fontWeight: 700, marginBottom: '0.5rem' }}>Monthly Hours + Cost Trend</div>
        {loading ? <Skeleton height={190} /> : monthly.length > 0
          ? <ChartWrapper option={trendOption} height={220} />
          : <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', padding: '2rem 0', textAlign: 'center' }}>No data</div>
        }
      </div>
      <div className="glass-raised" style={{ padding: '0.75rem', marginBottom: '0.85rem' }}>
        <div style={{ fontSize: '0.76rem', fontWeight: 700, marginBottom: '0.5rem' }}>Quick Focus Projects</div>
        {loading ? <Skeleton height={120} /> : topProjects.length > 0 ? (
          <div style={{ overflow: 'auto' }}>
            <table className="dm-table">
              <thead>
                <tr><th>Project</th><th>Progress</th><th>Actual Hrs</th><th>Remaining Hrs</th><th>Actual Cost</th></tr>
              </thead>
              <tbody>
                {topProjects.map(p => {
                  const pp = Number(p.percent_complete || 0);
                  return (
                    <tr key={p.id}>
                      <td style={{ fontWeight: 500, maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name || p.id}</td>
                      <td style={{ minWidth: 130 }}><div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><ProgressBar pct={pp} /><span style={{ fontSize: '0.68rem', fontWeight: 600, color: pctColor(pp), whiteSpace: 'nowrap' }}>{pp.toFixed(0)}%</span></div></td>
