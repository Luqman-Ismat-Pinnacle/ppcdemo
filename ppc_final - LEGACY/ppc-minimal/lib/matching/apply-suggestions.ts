...
export async function applyMapping(
  ...
  const count = await execute(
    `UPDATE hour_entries
        SET mpp_phase_task = $1,
            updated_at = NOW()
      WHERE project_id = $2
        AND LOWER(COALESCE(NULLIF(TRIM(phase), ''), 'unphased')) = LOWER(COALESCE(NULLIF(TRIM($3), ''), 'unphased'))
        AND COALESCE(mpp_phase_task, '') IS DISTINCT FROM COALESCE($1, '')`,
    [mppTarget, projectId, hourPhase],
  );
  return count;
}
...
export async function bulkApply(
  projectId: string,
  mappings: Array<{ hourPhase: string; mppTarget: string }>,
): Promise<number> {
  let total = 0;
  for (const m of mappings) {
    total += await applyMapping(projectId, m.hourPhase, m.mppTarget);
  }
  return total;
}
