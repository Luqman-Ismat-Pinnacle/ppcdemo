...
export function runMultiGateMatch(
  ...
  const subTasksByProject = new Map<string, Candidate[]>();
  ...
  for (const st of mppSubTasks) {
    if (!subTasksByProject.has(st.project_id)) subTasksByProject.set(st.project_id, []);
    subTasksByProject.get(st.project_id)!.push({
      id: st.id,
      name: st.name,
      normName: canonicalize(st.name),
      taskId: st.task_id,
      phaseId: st.phase_id,
      unitId: st.unit_id,
    });
  ...
  for (const h of hours) {
    ...
    const subTaskCands = subTasksByProject.get(pid) || [];
    ...
    if (!mpp) {
      ...
      if (preferText) {
        // Treat phase/task/sub-task names as a single interchangeable target pool.
        const primary = [...subTaskCands, ...taskCands, ...phaseCands];
    ...
    if (!mpp && wd) {
      const all = [...subTaskCands, ...taskCands, ...phaseCands];
