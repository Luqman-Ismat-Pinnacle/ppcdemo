...
/**
 * User Context for ppc-minimal.
 * Auth0 handles authentication. After login, we match the user's email
 * against the employees table and access-control overrides (Mauricio=COO, Angel=PCL).
 * Default view is based on resolved role.
 */

import React, { createContext, useContext, useState, useEffect, ReactNode } from 'react';
import { useUser as useAuth0User } from '@auth0/nextjs-auth0/client';
import { resolveRoleForIdentity, canSwitchViews } from '@/lib/access-control';
    ...
    const matchEmployee = async () => {
      ...
      if (!email) {
        setEnrichedUser({
          ...
          email: '',
          role: resolveRoleForIdentity({ email: auth0User.email || '', fallbackRole: 'PCA' }),
          initials: getInitials(auth0User.name || 'User'),
          canSwitchViews: canSwitchViews(auth0User.email || ''),
        });
        setFetched(true);
        return;
      }
      ...
      try {
        ...
        if (data.success && data.employee) {
          ...
          setEnrichedUser({
            ...
            canSwitchViews: canSwitchViews(data.employee.email || auth0User.email || ''),
