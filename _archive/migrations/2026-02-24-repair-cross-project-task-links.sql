...
-- SELECT COUNT(*) AS cross_project_parents
-- FROM tasks child JOIN tasks parent ON parent.id = child.parent_task_id
-- WHERE COALESCE(child.project_id, '') <> COALESCE(parent.project_id, '');
--
-- SELECT COUNT(*) AS cross_project_dependencies
-- FROM task_dependencies td
-- JOIN tasks pred ON pred.id = td.predecessor_task_id
-- JOIN tasks succ ON succ.id = td.successor_task_id
-- WHERE COALESCE(pred.project_id, '') <> COALESCE(succ.project_id, '');
