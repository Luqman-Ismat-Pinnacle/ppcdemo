...
export const DATA_KEY_TO_TABLE: Record<string, TableName> = {
  ...
  chargecodes: TABLES.CHARGE_CODES,
  projectHealth: TABLES.PROJECT_HEALTH,
  changeLog: TABLES.CHANGE_LOG,
};
 ...
 */
export function toSupabaseFormat<T extends Record<string, unknown>>(obj: T): Record<string, unknown> {
...
export async function fetchAll<T>(table: TableName): Promise<T[]> {
  if (!isSupabaseConfigured()) {
    console.warn(`Supabase not configured. Cannot fetch from ${table}.`);
    return [];
  }
 ...
 */
export async function insertRecords<T extends Record<string, unknown>>(
  table: TableName,
  records: T[]
 ...
 */
export async function syncTable<T extends Record<string, unknown>>(
  dataKey: string,
  records: T[]
): Promise<{ success: boolean; count: number; error?: string }> {
