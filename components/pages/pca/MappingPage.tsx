...
type MppSubTask = { id: string; name: string; task_id: string | null; phase_id: string | null };
...
export default function MappingPage() {
  ...
  const [mppSubTasks, setMppSubTasks] = useState<MppSubTask[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [mapR, wdR, uR, pR, tR, stR] = await Promise.all([
        ...
        fetch(`/api/tables/sub_tasks?project_id=${pid}&limit=4000`, { cache: 'no-store' }).then(r =>
          r.json(),
        ),
      ]);
      ...
      setMppSubTasks(
        (stR.rows || []).map((r: Record<string, unknown>) => ({
          id: str(r.id),
          name: str(r.name),
          task_id: r.task_id ? str(r.task_id) : null,
          phase_id: r.phase_id ? str(r.phase_id) : null,
        })),
      );
      setWdPhases(
        ...
        >
          ...
          >
            ...
            {leftTree.map((g, gIdx) => {
              ...
              return (
                <div key={`unit-${gIdx}-${g.unit}`} style={{ marginBottom: 2 }}>
                  {/* Unit row */}
                  <div
                    onClick={() => toggle(key)}
                    style={{
                  ...
                  {open &&
                    g.items.map((item, itemIdx) => {
                      ...
                      return (
                        <div
                          key={`phase-${g.unit}-${item.name}-${itemIdx}`}
                          style={{
          ...
          >
            ...
            {suggestions.map((s, sIdx) => {
              ...
              return (
                <div
                  key={`sug-${s.hourPhase}-${s.mppTargetId}-${sIdx}`}
                  style={{
                    display: 'flex',
            ...
            {unmatched.length > 0 && (
              <>
                ...
                {unmatched.map((b, bIdx) => (
                  <div
                    key={`unmatched-${b.phase}-${bIdx}`}
                    style={{
          ...
          >
            <div style={hdr}>
              ...
              <span style={muted66}>
                {mppUnits.length}U · {mppPhases.length}P · {mppTasks.length}T · {mppSubTasks.length}ST
              </span>
            </div>
