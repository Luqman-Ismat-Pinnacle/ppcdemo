...
function TaskCard({
  ...
  task: SprintTask;
  onRemove: (_taskId: string) => void;
  onOpen: (_task: SprintTask) => void;
}) {
  ...
  return (
    <div
      ...
      draggable
      onDragStart={(e) => {
        e.dataTransfer.setData('text/task-id', task.task_id);
        e.dataTransfer.effectAllowed = 'move';
      }}
      onClick={() => onOpen(task)}
      style={{ padding: '0.5rem 0.6rem', marginBottom: '0.4rem', fontSize: '0.75rem', cursor: 'grab' }}
      title="Drag to move lane, click to edit"
    >
...
function BoardColumn({
  ...
  return (
    <div
      ...
      style={{ padding: '0.65rem', minHeight: 200, display: 'flex', flexDirection: 'column' }}
      onDragOver={(e) => {
        e.preventDefault();
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {Array.from(grouped.entries()).map(([epic, epicTasks]) => (
          <div key={epic}>
            ...
            {epicTasks.map(t => (
              <TaskCard key={t.task_id} task={t} onRemove={onRemove} onOpen={onOpen} />
            ))}
          </div>
        ))}
      </div>
...
export default function SprintPage() {
  ...
  const loadData = useCallback(async (preserveSprint?: string) => {
    ...
    try {
      ...
      setEmployees((data.employees || []).map((e: Record<string, unknown>) => ({
        id: String(e.id || ''),
  ...
  const moveTaskToLane = async (taskId: string, lane: BoardLane) => {
    const percentComplete = lane === 'done' ? 100 : lane === 'in_progress' ? 50 : 0;
    await postAction({ action: 'updateTask', taskId, percentComplete });
    await loadData(selectedSprint);
  };

  const openTaskEditor = (task: SprintTask) => {
    setEditingTask(task);
  ...
  return (
    <div>
      ...
      {/* Create Sprint Form */}
      {showCreate && (
        <div className="glass" style={{ padding: '0.75rem', marginBottom: '0.75rem', overflow: 'visible', position: 'relative', zIndex: 10 }}>
            ...
            <div className="glass kpi-card">
              ...
              <div className="kpi-value">
                ...
                <div style={{ marginTop: 4, height: 4, borderRadius: 2, background: 'rgba(255,255,255,0.08)', width: '100%' }}>
                  <div style={{ width: `${Math.min(capacityPct, 100)}%`, height: '100%', borderRadius: 2, background: capacityPct > 100 ? 'var(--color-error, #EF4444)' : 'var(--color-accent, #8B5CF6)', transition: 'width 0.3s' }} />
                </div>
              </div>
            </div>
          </div>
      ...
      {editingTask && (
        ...
        >
          <div
            ...
            style={{ width: 'min(560px, 100%)', padding: '0.85rem', maxHeight: '90vh', overflow: 'auto' }}
            onClick={(e) => e.stopPropagation()}
          >
            <div style={{ fontSize: '0.84rem', fontWeight: 700, marginBottom: 4 }}>Edit Sprint Task</div>
            <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginBottom: 10 }}>{editingTask.task_name}</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div>
                <label style={{ fontSize: '0.68rem', color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>Assign Employee</label>
                <SearchableSelect
              ...
              <div>
                <label style={{ fontSize: '0.68rem', color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>Or custom resource</label>
                <input value={editResource} onChange={(e) => setEditResource(e.target.value)} style={inputStyle} />
              </div>
              <div>
                <label style={{ fontSize: '0.68rem', color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>Progress %</label>
                <input type="number" min={0} max={100} value={editPercent} onChange={(e) => setEditPercent(Number(e.target.value || 0))} style={inputStyle} />
              </div>
              <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
              ...
              <button className="btn btn-accent" onClick={saveTaskEditor} disabled={actionLoading}>
