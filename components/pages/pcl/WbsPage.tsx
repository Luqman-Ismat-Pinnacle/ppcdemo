      ...
      {!loading && items.length > 0 && (
        <div className="glass-solid" style={{ padding: '0.5rem', overflow: 'hidden', height: 'calc(100vh - 170px)', display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search WBS..." style={{ minWidth: 150, maxWidth: 190, background: 'var(--glass-bg)', color: 'var(--text-primary)', border: '1px solid var(--glass-border)', borderRadius: 8, padding: '0.3rem 0.45rem', fontSize: '0.68rem' }} />
            <div style={{ display: 'flex', gap: 2, background: 'rgba(0,0,0,0.2)', borderRadius: 8, padding: 2 }}>
              {(['week', 'month', 'quarter', 'year'] as Interval[]).map(iv => (
                <button key={iv} type="button" className="btn" onClick={() => setInterval(iv)} style={{ ...flatControlBtn, background: interval === iv ? 'rgba(99,102,241,0.26)' : 'rgba(255,255,255,0.02)', color: interval === iv ? '#eef2ff' : 'var(--text-secondary)', fontWeight: interval === iv ? 700 : 600, minWidth: 50, textTransform: 'capitalize' }}>{iv}</button>
              ))}
            </div>
            ...
            <select value={typeFilter} onChange={e => setTypeFilter(e.target.value as typeof typeFilter)} style={{ background: 'var(--glass-bg)', color: 'var(--text-primary)', border: '1px solid var(--glass-border)', borderRadius: 8, padding: '0.28rem 0.45rem', fontSize: '0.68rem' }}>
            ...
            <select value={progressFilter} onChange={e => setProgressFilter(e.target.value as typeof progressFilter)} style={{ background: 'var(--glass-bg)', color: 'var(--text-primary)', border: '1px solid var(--glass-border)', borderRadius: 8, padding: '0.28rem 0.45rem', fontSize: '0.68rem' }}>
          ...
          <div ref={hostRef} style={{ flex: 1, minHeight: 420, display: 'flex', border: '1px solid var(--glass-border)', borderRadius: 10, overflow: 'hidden' }}>
            ...
            <div ref={rightPaneRef} style={{ position: 'relative', display: 'flex', flex: 1, minWidth: 0, minHeight: 0, height: '100%', background: 'linear-gradient(180deg, rgba(10,12,16,0.56) 0%, rgba(8,10,13,0.44) 100%), repeating-linear-gradient(0deg, rgba(148,163,184,0.04) 0 1px, rgba(0,0,0,0) 1px 34px)' }}>
              <div ref={timelineRef} onMouseDown={onTimelineMouseDown} onMouseMove={onTimelineMouseMove} onMouseUp={onTimelineMouseUp} onMouseLeave={onTimelineMouseUp} style={{ position: 'relative', flex: 1, minHeight: 0, overflowX: 'auto', overflowY: 'hidden', cursor: isPanning ? 'grabbing' : 'grab' }}>
