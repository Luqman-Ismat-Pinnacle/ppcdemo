...
type WbsRow = {
  id: string; parent_id: string | null; type: string; level: number; has_children: boolean;
  project_id: string; unit_id: string; phase_id: string; task_id: string;
  wbs_code: string; name: string; resource_name: string;
  start_date: string | null; end_date: string | null;
  baseline_start: string | null; baseline_end: string | null;
  days_required: number; baseline_hours: number; actual_hours: number;
  remaining_hours: number; work: number; baseline_cost: number;
  actual_cost: number; remaining_cost: number; schedule_cost: number;
  cpi: number; efficiency: number; percent_complete: number;
  predecessor_ids: string[]; predecessor_name?: string;
  predecessor_task_id?: string; relationship?: string; lag_days?: number;
  total_float: number; is_critical: boolean; is_milestone?: boolean;
  comments?: string;
  source_table?: string;
...
type RowGeom = { id: string; rowIndex: number; top: number; height: number; center: number; bottom: number };
...
function fmtDate(v: string | null) { if (!v) return '-'; const d = parseFlexibleDate(v); return !d ? '-' : d.toLocaleDateString('en-US'); }
function parseDate(v: string | null | undefined): Date | null { return parseFlexibleDate(v); }
...
export default function PclWbsPage({ apiBase = '/api/pcl/wbs', roleHeader = 'PCL', title = 'WBS Gantt' }: WbsPageProps = {}) {
  ...
  React.useEffect(() => {
    ...
    fetch(url, { cache: 'no-store' })
      ...
      .then(d => {
        ...
        rows.forEach(r => { if (r.has_children && r.level <= 3) defaultExpanded.add(r.id); });
  ...
  React.useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      setSplit(Math.max(30, Math.min(70, ((e.clientX - rect.left) / rect.width) * 100)));
    };
  ...
  const captureRowGeometry = useCallback(() => {
    ...
    for (let i = 0; i < displayed; i += 1) {
      ...
      next.push({ id: data.id, rowIndex: i, top, height, center: top + height / 2, bottom: top + height });
    }
    setRowGeom(prev => {
      if (prev.length === next.length) {
        ...
        for (let i = 0; i < next.length; i++) {
          ...
          if (!a || !b || a.id !== b.id || a.top !== b.top || a.height !== b.height) { same = false; break; }
        }
        if (same) return prev;
      }
      return next;
  ...
  const scheduleGridSync = useCallback(() => {
    ...
    syncRafRef.current = requestAnimationFrame(() => { syncRafRef.current = null; syncVScrollFromGrid(); captureRowGeometry(); });
  ...
  React.useEffect(() => {
    ...
    scheduleGridSync();
    return () => { viewport.removeEventListener('scroll', onScroll); if (gridBodyViewportRef.current === viewport) gridBodyViewportRef.current = null; };
  ...
  const filteredRows = useMemo(() => {
    ...
    return items.filter(r => {
      ...
      if (!q) return true;
      return [r.wbs_code, r.name, r.type, r.resource_name, String(r.predecessor_ids?.join(',') || '')].join(' ').toLowerCase().includes(q);
    });
  ...
  const rows = useMemo(() => {
    ...
    filteredRows.forEach(r => { const key = parentKey(r.parent_id); if (!byParent.has(key)) byParent.set(key, []); byParent.get(key)!.push(r); });
    const sortRows = (arr: WbsRow[]) => [...arr].sort((a, b) => a.level !== b.level ? a.level - b.level : String(a.name || '').localeCompare(String(b.name || '')));
    const out: WbsRow[] = [];
    ...
    const walk = (parentId: string) => {
      ...
      kids.forEach((row) => {
        ...
        visited.add(row.id);
        out.push(row);
        if (row.has_children && expandedIds.has(row.id)) walk(row.id);
      });
    };
    walk('__root__');
    return out;
  ...
  const range = useMemo(() => {
    ...
    if (!dates.length) { const now = new Date(); return { min: new Date(now.getFullYear(), now.getMonth() - 6, 1), max: new Date(now.getFullYear(), now.getMonth() + 6, 1) }; }
    return { min: new Date(Math.min(...dates) - 28 * DAY_MS), max: new Date(Math.max(...dates) + 28 * DAY_MS) };
  }, [rows]);
  ...
  const axisTicks = useMemo(() => {
    const ticks: Date[] = [];
    const { min, max } = range;
    if (interval === 'week') { const c = new Date(min.getFullYear(), min.getMonth(), min.getDate()); while (c <= max) { ticks.push(new Date(c)); c.setDate(c.getDate() + 7); } return ticks; }
    if (interval === 'month') { const c = new Date(min.getFullYear(), min.getMonth(), 1); while (c <= max) { ticks.push(new Date(c)); c.setMonth(c.getMonth() + 1); } return ticks; }
    if (interval === 'quarter') { const sq = Math.floor(min.getMonth() / 3) * 3; const c = new Date(min.getFullYear(), sq, 1); while (c <= max) { ticks.push(new Date(c)); c.setMonth(c.getMonth() + 3); } return ticks; }
    const c = new Date(min.getFullYear(), 0, 1); while (c <= max) { ticks.push(new Date(c)); c.setFullYear(c.getFullYear() + 1); } return ticks;
  }, [range, interval]);
  ...
  const taskIndexById = useMemo(() => {
    ...
    rows.forEach((r, i) => { if (r.type === 'task' || r.type === 'sub_task') { if (r.task_id) m.set(String(r.task_id), i); if (r.id) m.set(String(r.id), i); } });
    return m;
  }, [rows]);
  ...
  const timelineRows = useMemo<RowGeom[]>(
    () => rows.map((r, i) => {
      ...
      return { id: r.id, rowIndex: i, top, height, center: top + height / 2, bottom: top + height };
    }),
    [rows, rowGeomById],
  );
  ...
  const gridColumns = useMemo<ColDef<WbsRow>[]>(() => ([
    {
      field: 'name', headerName: 'Name', minWidth: 300, width: 340,
      cellRenderer: (p: { data?: WbsRow; value?: unknown }) => {
        const row = p.data; if (!row) return null;
        const canExpand = row.has_children; const expanded = expandedIds.has(row.id);
        return (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, paddingLeft: Math.max(0, row.level * 14), minWidth: 0, cursor: canExpand ? 'pointer' : 'default' }}
            onClick={() => { if (!canExpand) return; setExpandedIds(prev => { const next = new Set(prev); if (next.has(row.id)) next.delete(row.id); else next.add(row.id); return next; }); }}>
            <span style={{ width: 12, opacity: 0.75 }}>{canExpand ? (expanded ? '▾' : '▸') : ''}</span>
            <span style={{ opacity: 0.75 }}>{row.type === 'portfolio' ? '◉' : row.type === 'customer' ? '◎' : row.type === 'site' ? '◇' : row.type === 'project' ? '⬢' : row.type === 'unit' ? '▣' : row.type === 'phase' ? '▤' : row.type === 'task' ? '▪' : '▫'}</span>
            <span style={{ fontWeight: 600, color: '#e5e7eb', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{String(p.value || '')}</span>
          </div>
        );
      },
    },
  ...
  const fitTimeline = () => { const host = timelineRef.current; if (!host) return; setPxPerDay(Math.max(0.25, Math.min(12, (host.clientWidth - 120) / Math.max(1, totalDays)))); };

  const onTimelineMouseDown = (e: React.MouseEvent<HTMLDivElement>) => { if (e.button !== 0) return; const el = timelineRef.current; if (!el) return; setIsPanning(true); panStartXRef.current = e.clientX; panStartLeftRef.current = el.scrollLeft; };
  const onTimelineMouseMove = (e: React.MouseEvent<HTMLDivElement>) => { if (!isPanning) return; const el = timelineRef.current; if (!el) return; el.scrollLeft = Math.max(0, panStartLeftRef.current - (e.clientX - panStartXRef.current)); };
  ...
  const showBarTip = (evt: { evt: MouseEvent }, row: WbsRow) => {
    ...
    setBarTip({
      x: evt.evt.clientX - rect.left + 12, y: evt.evt.clientY - rect.top - 12,
      name: row.name || row.wbs_code || row.id, type: row.type, start: fmtDate(row.start_date), end: fmtDate(row.end_date),
      baseline: `${fmtDate(row.baseline_start)} -> ${fmtDate(row.baseline_end)}`, progress: asNum(row.percent_complete),
      predecessor: row.predecessor_name || row.predecessor_task_id || '-', rel: row.relationship || '-', lag: asNum(row.lag_days),
  ...
  return (
    <div>
      ...
      <p className="page-subtitle">Hierarchical WBS grid with Konva timeline, dependencies, and full planning columns.</p>
      ...
      {!loading && items.length > 0 && (
        <div className="glass-solid" style={{ padding: '0.5rem', overflow: 'hidden', height: 'calc(100vh - 170px)', display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search WBS..." style={{ minWidth: 170, maxWidth: 240, background: 'rgba(255, 255, 255, 0.04)', color: 'var(--text-primary)', border: '1px solid var(--glass-border)', borderRadius: 8, padding: '0.4rem 0.55rem', fontSize: '0.74rem' }} />
            <div style={{ display: 'flex', gap: 2, background: 'rgba(0,0,0,0.2)', borderRadius: 8, padding: 2 }}>
              {(['week', 'month', 'quarter', 'year'] as Interval[]).map(iv => (
                <button key={iv} type="button" className="btn" onClick={() => setInterval(iv)} style={{ ...controlBtnStyle, background: interval === iv ? 'rgba(99,102,241,0.26)' : undefined, color: interval === iv ? '#eef2ff' : undefined, minWidth: 58, textTransform: 'capitalize' }}>{iv}</button>
              ))}
            </div>
            ...
            <select value={typeFilter} onChange={e => setTypeFilter(e.target.value as typeof typeFilter)} style={{ background: 'rgba(255, 255, 255, 0.04)', color: 'var(--text-primary)', border: '1px solid var(--glass-border)', borderRadius: 8, padding: '0.38rem 0.55rem', fontSize: '0.74rem' }}>
              <option value="all">All Types</option><option value="critical">Critical Only</option><option value="task">Tasks Only</option><option value="phase">Phases Only</option>
            </select>
            <select value={progressFilter} onChange={e => setProgressFilter(e.target.value as typeof progressFilter)} style={{ background: 'rgba(255, 255, 255, 0.04)', color: 'var(--text-primary)', border: '1px solid var(--glass-border)', borderRadius: 8, padding: '0.38rem 0.55rem', fontSize: '0.74rem' }}>
              <option value="all">All Progress</option><option value="not_started">Not Started</option><option value="in_progress">In Progress</option><option value="done">Done</option>
            </select>
          ...
          <div ref={hostRef} style={{ flex: 1, minHeight: 420, height: '100%', display: 'flex', border: '1px solid var(--glass-border)', borderRadius: 10, overflow: 'hidden' }}>
            <div style={{ width: `${split}%`, minWidth: 0 }}>
              <div ref={gridWrapRef} className="ag-theme-quartz wbs-grid-theme" style={{ width: '100%', height: '100%' }}>
                <AgGridReact<WbsRow> theme="legacy" rowData={rows} columnDefs={gridColumns} getRowId={p => p.data.id} animateRows={false} suppressScrollOnNewData rowHeight={ROW_H} headerHeight={HEADER_H} suppressRowClickSelection
                  onGridReady={(e: GridReadyEvent<WbsRow>) => { gridApiRef.current = e.api; scheduleGridSync(); }}
                  onBodyScroll={(e: BodyScrollEvent) => { if (e.direction === 'vertical') { if (e.top != null) { const top = Math.max(0, e.top); if (Math.abs(lastVScrollRef.current - top) > 0.5) { lastVScrollRef.current = top; setVScroll(top); } } else { scheduleGridSync(); } } }}
                  onModelUpdated={scheduleGridSync} onSortChanged={scheduleGridSync} onFilterChanged={scheduleGridSync} onCellValueChanged={saveInlineComment}
                  tooltipShowDelay={0}
                  tooltipHideDelay={15000}
                  defaultColDef={{ sortable: true, resizable: true, filter: true, tooltipValueGetter: (p) => varianceTooltip(p.data as WbsRow | undefined, String((p.colDef as { field?: string })?.field ?? ''), p.value) }} />
              </div>
            </div>
            ...
            <div ref={rightPaneRef} style={{ position: 'relative', display: 'flex', alignItems: 'stretch', flex: 1, minWidth: 0, minHeight: 0, height: '100%', background: 'linear-gradient(180deg, rgba(10,12,16,0.56) 0%, rgba(8,10,13,0.44) 100%), repeating-linear-gradient(0deg, rgba(148,163,184,0.04) 0 1px, rgba(0,0,0,0) 1px 34px)' }}>
              ...
              <div ref={timelineRef} onMouseDown={onTimelineMouseDown} onMouseMove={onTimelineMouseMove} onMouseUp={onTimelineMouseUp} onMouseLeave={onTimelineMouseUp} style={{ position: 'relative', flex: 1, height: '100%', minHeight: 0, overflowX: 'auto', overflowY: 'hidden', cursor: isPanning ? 'grabbing' : 'grab', visibility: hasRenderableTimelineRows ? 'visible' : 'hidden' }}>
                {isClient && (
                <Stage width={timelineWidth} height={stageHeight}>
                  <Layer clipX={0} clipY={HEADER_H} clipWidth={timelineWidth} clipHeight={Math.max(0, stageHeight - HEADER_H)}>
                    ...
                    {visibleTimelineRows.map(g => {
                      ...
                      const x1 = hasDates ? toX(s as Date) : 0; const x2 = hasDates ? toX(e as Date) : 0;
                      ...
                      return (
                        <React.Fragment key={`row-${g.rowIndex}-${g.id}`}>
                          ...
                          {showBaseline && r.baseline_start && r.baseline_end && (() => { const bs = parseDate(r.baseline_start); const be = parseDate(r.baseline_end); if (!bs || !be) return null; const bx1 = toX(bs); const bx2 = toX(be); if (!Number.isFinite(bx1) || !Number.isFinite(bx2)) return null; const barY = y + Math.max(6, Math.floor((laneH - (ROW_H - 16)) / 2)); const barH = Math.max(8, laneH - 16); return <Rect x={bx1} y={barY + Math.floor((barH - 6) / 2)} width={Math.max(3, bx2 - bx1)} height={6} cornerRadius={3} fill="rgba(148,163,184,0.58)" />; })()}
                          {hasDates && (() => { const barY = y + Math.max(6, Math.floor((laneH - (ROW_H - 16)) / 2)); const barH = Math.max(8, laneH - 16); const delta = Number(varianceFor(r, 'percent_complete')?.delta || 0); const hasVar = rowHasVariance(r); const varianceColor = delta > 0 ? '#22c55e' : delta < 0 ? '#ef4444' : '#a78bfa'; const finalColor = showVariance && hasVar ? varianceColor : color; return (<><Rect x={x1} y={barY} width={Math.max(6, x2 - x1)} height={barH} cornerRadius={4} fill={finalColor} opacity={showVariance && hasVar ? 0.4 : 0.22} stroke={showVariance && hasVar ? '#e2e8f0' : undefined} strokeWidth={showVariance && hasVar ? 1.2 : 0} onMouseEnter={evt => showBarTip(evt, r)} onMouseLeave={hideBarTip} /><Rect x={x1} y={barY} width={fillW} height={barH} cornerRadius={4} fill={finalColor} shadowBlur={showVariance && hasVar ? 10 : 0} shadowColor={showVariance && hasVar ? finalColor : undefined} onMouseEnter={evt => showBarTip(evt, r)} onMouseLeave={hideBarTip} /></>); })()}
                        </React.Fragment>
                      );
                    })}
                    {(() => {
                  ...
                  <Layer listening={false}>
                    ...
                    {axisTicks.map(tick => { const x = toX(tick); return (<React.Fragment key={`tick-${tick.toISOString()}`}><Line points={[x, 0, x, HEADER_H]} stroke="rgba(148,163,184,0.28)" strokeWidth={1} /><Text x={x + 3} y={12} text={formatTick(tick)} fill="#cbd5e1" fontSize={11} /></React.Fragment>); })}
